...
import { FEATURED_ID, TRACKS, getTrack } from "@/lib/music";
    ...
    <div className="space-y-10">
      <header className="space-y-4">
        ...
        <h1 className="max-w-3xl font-display text-[2.15rem] font-semibold leading-[1.15] text-ink sm:text-5xl">
          Nhạc chính của app là «Hương on Return» — mở đầu buổi và xuyên suốt lớp.
        </h1>
        <p className="max-w-2xl text-base text-muted-foreground">
          Tạo hình lấy từ nhân vật đất sét 3D của Bật Nghiệp. Bài hát lấy từ tuyển tập cùng tên trong kho
          nhạc: gắn sao, 2:30, dùng làm nhạc nền mặc định của toàn app.
        </p>
      </header>

      <section className="overflow-hidden rounded-xl border border-border bg-card shadow-soft">
        <div className="grid md:grid-cols-2">
          <div className="flex items-center justify-center bg-sunken p-6">
            <img
              src={assetUrl(featured.art)}
              alt="Lumina nghe nhạc với tai nghe, đang phát Hương on Return"
              className="aspect-square w-full max-w-xs object-contain"
            />
          </div>
          <div className="flex flex-col justify-between gap-5 p-5 sm:p-6">
            <div className="space-y-3">
              <Badge variant="default">Nhạc chính của app</Badge>
              <h2 className="text-xl font-semibold text-ink">
                {featured.title} · {featured.version}
              </h2>
              <p className="text-sm leading-relaxed text-muted-foreground">{featured.use}</p>
              <p className="text-xs text-muted-foreground">
                Phát mặc định khi mở app SV và GV — bản đầy đủ {featured.durationLabel} luôn có ở thanh
                nhạc dưới cùng, tua/tạm dừng được mọi lúc.
              </p>
            </div>
      ...
      <section className="space-y-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Cùng một tạo hình Lumina, năm bản từ kho nhạc. Mặc định là «Hương on Return»; các bản còn lại
            đổi theo nhịp buổi, không át thảo luận.
          </p>
        </div>
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex flex-wrap items-center gap-2">
                      ...
                      {t.featured ? <Badge variant="default">Nhạc chính</Badge> : null}
                    </span>
